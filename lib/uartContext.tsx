...
const UartContext = createContext<UartContextType>({ lastPacket: null, isDimmed: false })
...
export function UartProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    uartClientRef.current.connect((packet) => {
      ...
      if (packet.type === 'complete_packet' && packet.data) {
        ...
        console.log('🔍 UART packet received:', data)
  ...
  return (
    <UartContext.Provider value={{ lastPacket, isDimmed }}>
      {children}
